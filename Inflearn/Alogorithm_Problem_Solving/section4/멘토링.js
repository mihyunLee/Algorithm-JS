...
function solution(N, M, arr) {
  ...
  for (let i = 1; i <= N; i++) {
    for (let j = 1; j <= N; j++) {
      ...
      if (i === j) continue; // 같은 인덱스는 멘토-멘티 관계가 될 수 없다.

      for (let k = 0; k < M; k++) {
        const gi = arr[k].indexOf(i);
        const gj = arr[k].indexOf(j);

        if (gi < gj) count++; // 등수가 높을수록 인덱스가 작으므로 gi 인덱스가 gj 인덱스보다 작을 때 멘토-멘티 관계 성립
      }

      if (count === M) answer++; // M번의 테스트에서 멘토-멘티 관계가 성립되어야 최종 멘토-멘티 관계가 된다.
    }
  }
